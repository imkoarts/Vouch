"""Run an auditor-supplied materiality/proposition hybrid-semantic holdout.

The target is a frozen release candidate.
...
SUPPORTED_LEGACY_SCHEMA_VERSIONS = frozenset({"2.0", "3.0", "4.0"})
...
class HoldoutExpectation(BaseModel):
    ...
    required_human_function_domains: tuple[str, ...] = ()
    required_human_function_impacts: tuple[str, ...] = ()
    required_literal_proposition_types: tuple[str, ...] = ()
    required_predicate_scope_relations: tuple[str, ...] = ()
...
class HoldoutDocument(BaseModel):
    ...
    schema_version: str = Field(json_schema_extra={"const": CURRENT_SCHEMA_VERSION})
    holdout_id: str = Field(min_length=1)
    cases: tuple[HoldoutCase, ...] = Field(min_length=1)

    @field_validator("schema_version")
    @classmethod
    def require_current_schema(cls, value: str) -> str:
        if value != CURRENT_SCHEMA_VERSION:
            raise ValueError(f"schema_version must be the current version {CURRENT_SCHEMA_VERSION}")
        return value
...
def _inspection_summary(inspection: SemanticInspection) -> dict[str, Any]:
    ...
    return {
        ...
        "result_state_materiality": tuple(
            item.model_dump(mode="json") for item in inspection.result_state_materiality
        ),
        "human_function_impacts": tuple(
            item.model_dump(mode="json") for item in inspection.human_function_impacts
        ),
        "reply_propositions": tuple(
            item.model_dump(mode="json") for item in inspection.reply_propositions
        ),
        "reply_semantic_completeness": tuple(
            item.model_dump(mode="json") for item in inspection.reply_semantic_completeness
        ),
        "literal_proposition_frames": tuple(
            item.model_dump(mode="json") for item in inspection.literal_proposition_frames
        ),
        "predicate_scopes": tuple(
            item.model_dump(mode="json") for item in inspection.predicate_scopes
        ),
...
def _compare(case: HoldoutCase, observed: dict[str, Any]) -> tuple[str, ...]:
    ...
    subset_checks = (
        ...
        (
            expected.required_human_function_domains,
            tuple(item["function_domain"] for item in observed["human_function_impacts"]),
            "human_function_domains",
        ),
        (
            expected.required_human_function_impacts,
            tuple(item["impact"] for item in observed["human_function_impacts"]),
            "human_function_impacts",
        ),
        (
            expected.required_literal_proposition_types,
            tuple(item["proposition_type"] for item in observed["literal_proposition_frames"]),
            "literal_proposition_types",
        ),
        (
            expected.required_predicate_scope_relations,
            tuple(item["relation_to_parent"] for item in observed["predicate_scopes"]),
            "predicate_scope_relations",
        ),
    )
...
def migrate_legacy_document(raw: dict[str, Any]) -> tuple[dict[str, Any], tuple[str, ...]]:
    """Return current input unchanged or migrate supported legacy input directly to current."""

    if not isinstance(raw, dict):
        raise ValueError("holdout document must be a JSON object")
    source_version = raw.get("schema_version")
    if source_version == CURRENT_SCHEMA_VERSION:
        return raw, ()
    if source_version not in SUPPORTED_LEGACY_SCHEMA_VERSIONS:
        raise ValueError(f"unsupported holdout schema version: {source_version}")
    migrated = json.loads(json.dumps(raw))
    migrations: list[str] = [f"schema:{source_version}->{CURRENT_SCHEMA_VERSION}"]
