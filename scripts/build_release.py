...
TELEGRAM_TOKEN_PATTERN = re.compile(
    r"(?<![A-Za-z0-9])\d{8,12}:[A-Za-z0-9_-]{30,}"
)
...
NON_NULL_OPERATOR_PATTERN = re.compile(
    r"(?m)^\s*operator_user_id:\s*(?!null\s*$)[1-9]\d*\s*$"
)
...
def validate_manifest(manifest: Sequence[Path]) -> None:
    ...
    duplicates = sorted(
        path.as_posix()
        for path in set(normalized)
        if normalized.count(path) > 1
    )

    if forbidden:
        raise ReleaseArchiveError(
            f"Forbidden release paths: {', '.join(forbidden)}"
        )

    if duplicates:
        raise ReleaseArchiveError(
            f"Duplicate release paths: {', '.join(duplicates)}"
        )
...
def build_manifest(root: Path = ROOT) -> tuple[Path, ...]:
    ...
    if missing:
        raise ReleaseArchiveError(
            f"Required release files are missing: {', '.join(missing)}"
        )
    ...
    for relative in ALLOWED_BINARY_FILES:
        ...
        if not source.is_file():
            raise ReleaseArchiveError(
                f"Required release file is missing: {relative.as_posix()}"
            )
    ...
    for directory_name in ALLOWED_DIRECTORIES:
        ...
        if not directory.is_dir():
            raise ReleaseArchiveError(
                f"Required release directory is missing: {directory_name}"
            )

        for source in directory.rglob("*"):
            if source.is_symlink():
                raise ReleaseArchiveError(
                    f"Symlink is forbidden in release: {source}"
                )
    ...
    for relative in manifest:
        ...
        try:
            ...
            try:
                ...
                violations.append(
                    f"{relative.as_posix()}: text file is not UTF-8/ASCII"
                )
                continue

        if CYRILLIC_PATTERN.search(text):
            violations.append(
                f"{relative.as_posix()}: contains Cyrillic text"
            )

        if WINDOWS_USER_PATH_PATTERN.search(text):
            violations.append(
                f"{relative.as_posix()}: contains a local Windows user path"
            )

        if (
            relative.as_posix() == "config/runtime.yml"
            and NON_NULL_OPERATOR_PATTERN.search(text)
        ):
            violations.append(
                "config/runtime.yml: Telegram operator_user_id must be null"
            )
        ...
        if not relative.parts or relative.parts[0] != "tests":
            if TELEGRAM_TOKEN_PATTERN.search(text):
                violations.append(
                    f"{relative.as_posix()}: "
                    "contains a Telegram token-like value"
                )

            if API_SECRET_PATTERN.search(text):
                violations.append(
                    f"{relative.as_posix()}: "
                    "contains an API key-like value"
                )

            if AUTHENTICATED_PROXY_PATTERN.search(text):
                violations.append(
                    f"{relative.as_posix()}: contains proxy credentials"
                )

    if violations:
        raise ReleaseArchiveError(
            "Public release content violations: "
            + "; ".join(violations)
        )
        ...
        with zipfile.ZipFile(
            ...
            for relative in manifest:
                ...
                if root not in source.parents:
                    raise ReleaseArchiveError(
                        f"Release source escaped project root: {relative}"
                    )
